...
void LCD_display(char upperLCD[], char lowerLCD[]){
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print(upperLCD);
  delay(500); 
...
void gpsdump(TinyGPS &gps){
  ...
  char Flat[10];
  sprintf(Flat, "%d", flat);
  writeToFile("Lat: "); writeToFile(Flat); writeToFile("\n");
  char Flon[10];
  sprintf(Flon, "%d", flon);
  writeToFile("Lon: "); writeToFile(Flon); writeToFile("\n");
...
void setup() {
  ...
  // LCD
  LCD_initilize();
  LCD_display("Project by:", "M Hamza");
